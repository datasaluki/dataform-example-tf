...
2. dataform - Used to provision a dataform repository, connect it to a git repo using SSH and set up release and workflow configurations
...
2. Create a backend.conf file to configure terraform to store state in the Google Cloud Storage bucket
3. Update the terraform.tfvars file if you want to make any changes
4. Create a terraform.tfvars file containing any variables you want to set
...
prefix = "terraform/state/dataform-example/source-infra"
```
...
1. Export your google cloud project id (e.g. `export GOOGLE_CLOUD_PROJECT="YOUR_PROJECT"`)
2. Create a backend.conf file to configure terraform to store state in the Google Cloud Storage bucket
3. Create a terraform.tfvars file and set the git connection and other variables
...
```hcl
bucket = "my-secret-terraform-state-bucket"
prefix = "terraform/state/dataform-example/dataform"
```
